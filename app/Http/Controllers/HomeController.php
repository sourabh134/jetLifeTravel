...
class HomeController extends Controller
{
    ...
    function oneWayFlightSearch($request, $journeyType)
    {
    ...
    {
        $data = $this->oneWayFlightSearch($request, $request->trip);
        // $journeyType = $request->trip;
        // if ($journeyType == "oneway") {
        //     $options = $request->options;
        //     $passengers = $request->passengers;
        ...
        $from = $data['from'];
        $to = $data['to'];
        $airlineStats = $data['airlineStats'];
        $stopStats = $data['stopStats'];
        $refundStats = $data['refundStats'];
    ...
    public function roundTripFlightSearchResult(Request $request)
    {
        echo "<pre>";
        print_r($request->all());
        // die;
        ...
        if ($journeyType == "roundtrip") {
            ...
            foreach ($pairs1 as $pair) {
                list($key, $value) = explode(':', $pair);
                $extracted1[$key] = $value;
            }

            // Access individual values for leg 1
            $from1 = $extracted1['from'];
            $to1 = $extracted1['to'];
            $departure1 = date('Y-m-d', strtotime($extracted1['departure']));
            $fromType1 = $extracted1['fromType'];
            $toType1 = $extracted1['toType'];

            // Extract leg 2 values
            $extracted2 = [];
            foreach ($pairs2 as $pair) {
                list($key, $value) = explode(':', $pair);
                $extracted2[$key] = $value;
            }

            // Access individual values for leg 2
            $from2 = $extracted2['from'];
            $to2 = $extracted2['to'];
            $departure2 = date('Y-m-d', strtotime($extracted2['departure']));
            $fromType2 = $extracted2['fromType'];
            $toType2 = $extracted2['toType'];


            // Assuming fromType and toType are the same for both legs
            // If they differ, you can extract them similarly as above
            ...
            // ... (same as in one-way search)
            $oneWay = $this->oneWayFlightSearch($request, "oneway");
            ...
            // Call API for round trip search
            // ... (similar to one-way search, but with both legs)

            $flightsearchRoundTrip = APIService::flightSearchRoundTrip($journeyType, $departure1, $departure2, $to2, $from2, $cabinClass, $adults, $children, $infantInLap);

            echo "<pre>";
            print_r($flightsearchRoundTrip);
            die;

        }
        // return view('front.round-trip-flight-search-result');
    }
}
