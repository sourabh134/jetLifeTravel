...
class APIService
{
    ...
    public static function revalidate($session_id, $fare_source_code)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://travelnext.works/api/aeroVE5/revalidate',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        $response = curl_exec($curl);
        curl_close($curl);
        $revalidate = json_decode($response, true);
    }

    public static function fare_rules($session_id, $fare_source_code)
    {
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://travelnext.works/api/aeroVE5/fare_rules',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
    ...
    public static function flightsearchmultiplecity($legJson, $cabinClass, $adults, $children, $infantInLap)
    {
        $legs = json_decode($legJson, true);
        $converted = [];

        foreach ($legs as $leg) {
            $converted[] = [
                'departureDate' => date('Y-m-d', strtotime(str_replace('/', '-', $leg['date']))),
                'airportOriginCode' => $leg['from'],
                'airportDestinationCode' => $leg['to']
            ];
        }

        $leg = json_encode($converted, JSON_PRETTY_PRINT);
        $curl = curl_init();
        ...
        curl_setopt_array($curl, array(
            ...
            "user_password": "jetlifeglobalTest@2025",
            "access": "Test",
            "ip_address": "122.161.50.162",
            "requiredCurrency": "USD",
            "journeyType": "Circle",
            "OriginDestinationInfo": ' . $leg . ',
            "class": "' . ucfirst(strtolower($cabinClass)) . '",
            "adults": ' . $adults . ',
            "childs": ' . $children . ',
            "infants": ' . $infantInLap . '
